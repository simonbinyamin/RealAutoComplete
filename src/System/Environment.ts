...
export const EnvironmentDisposable = vscode.languages.registerCompletionItemProvider(
    'csharp',
    {
        provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
            const linePrefix = document.lineAt(position).text.substr(0, position.character);
            if (!linePrefix.endsWith('Environment.')) {
                return undefined;
            }
            
            var json = require('../Json/trial.json');
            var stringMethods = json.System.Environment.Methods as string[];
