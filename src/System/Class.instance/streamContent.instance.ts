...
export const StreamContentInstanceDisposable = vscode.languages.registerCompletionItemProvider(
    'csharp',
    {
        ...
        provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Promise<vscode.CompletionItem[]> 
        {
            ...
            //StreamContent x = new StreamContent
            if (document.getText().includes("StreamContent ")
            ) {
                for (let i = 1; i < document.lineCount; i++) {
                    var line = document.lineAt(i).text;
                    if(line.includes("StreamContent ") && line.indexOf(";") !== -1 && line.includes("new")
                    ) {
                        var lineArray = line.split(" ");
                        lineArray = lineArray.filter(e => String(e).trim());
                        listVariables.push(lineArray[1]);
            ...
            // var x =  new StreamContent
            if (document.getText().includes("var ") && document.getText().includes("new StreamContent")
            ) {
                for (let i = 1; i < document.lineCount; i++) {
                    var line = document.lineAt(i).text;
                    if(line.includes("var ") && line.includes("new StreamContent")
                    ) {
                        var lineArray = line.split(" ");
                        lineArray = lineArray.filter(e => String(e).trim());
                        listVariables.push(lineArray[1]);
            ...
            // method(StreamContent x,StreamContent x, StreamContent x)
            if (document.getText().indexOf("StreamContent") !== -1
            ) {
                for (let i = 1; i < document.lineCount; i++) {
                    var line = document.lineAt(i).text;
