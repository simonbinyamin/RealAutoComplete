        ...
        provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Promise<vscode.CompletionItem[]> 
        {
            ...
            if (document.getText().includes("DateTime ")) {
                for (let i = 1; i < document.lineCount; i++) {
                    ...
                    if(line.includes("DateTime ") && line.indexOf(";") !== -1 && line.includes("new")
                    ) {
            ...
            // var x =  new DateTime
            if (document.getText().includes("var ") && document.getText().includes("new DateTime")
            ) {
                for (let i = 1; i < document.lineCount; i++) {
                    var line = document.lineAt(i).text;
                    if(line.includes("var ") && line.includes("new DateTime")
                    ) {
                        var lineArray = line.split(" ");
                        lineArray = lineArray.filter(e => String(e).trim());
                        listVariables.push(lineArray[1]);
            ...
            if (document.getText().indexOf("DateTime") !== -1
            ) {
                for (let i = 1; i < document.lineCount; i++) {
                    var line = document.lineAt(i).text;
                    if(line.indexOf("DateTime") !== -1 && !line.includes("var") && line.indexOf(";") === -1
                    ) {
                        var lineArray = line.split(" ");
                        lineArray = lineArray.filter(e => String(e).trim());

                        let index1 = lineArray.findIndex(str => str.includes("DateTime"))
